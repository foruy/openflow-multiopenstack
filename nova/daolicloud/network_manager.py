...
class SimpleManager(NetworkManager):
    ...
    def allocate_fixed_ip(self, context, instance_id, network, **kwargs):
        ...
        if kwargs.get('vpn', None):
            ...
            if not address:
                raise exception.InvalidAddress(address=address)
        ...
        fip = objects.FixedIP.fixed_ip_add(context, str(address), instance_id,
                                           network_id=network['id'], reserved=reserved,
                                           allocated=True, virtual_interface_id=vif.id)

        instance = objects.Instance.get_by_uuid(context, instance_id)

        LOG.info('Allocated fixed ip %s on network %s', address,
