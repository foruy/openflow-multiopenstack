...
def get_available_images(request, zone_id, project_id=None, images_cache=None):
    ...
    if 'public_images' not in images_cache:
        public = {"is_public": True,
                  "status": "active"}
        try:
            images, _more = api.proxy.image_list_detailed(
                request, zone_id, filters=public)
    ...
    if project_id not in images_by_project:
        owner = {"property-owner_id": project_id,
                 "status": "active"}
        try:
            owned_images, _more = api.proxy.image_list_detailed(
                request, zone_id, filters=owner)
