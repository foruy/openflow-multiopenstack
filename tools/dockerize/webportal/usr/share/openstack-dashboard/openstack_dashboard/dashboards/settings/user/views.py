...
def set_lang(request):
    data = {
            'language': request.GET.get('language', request.session.get(
                settings.LANGUAGE_COOKIE_NAME,
                request.COOKIES.get(settings.LANGUAGE_COOKIE_NAME,
                                    request.LANGUAGE_CODE))),
            'timezone': request.GET.get('timezone', request.session.get(
                'django_timezone',
                request.COOKIES.get('django_timezone', 'UTC'))),
            'pagesize': request.GET.get('pagesize',
                utils.get_page_size(request))}

    return user_forms.handle(request, request.GET.get('next', '/'), data)
