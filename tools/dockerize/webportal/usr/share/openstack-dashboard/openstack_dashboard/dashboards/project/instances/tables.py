...
class TerminateInstance(tables.BatchAction):
    ...
    def allowed(self, request, instance=None):
        ...
        if instance is not None and instance.status == UNKNOWN:
            return False
        return not is_deleting(instance)

    def action(self, request, obj_id):
        api.proxy.server_delete(request, obj_id)
        #bill_api.delete_container_count(request.session["user_id"], obj_id, obj_id)
...
class LaunchLink(tables.LinkAction):
    ...
    #url = "horizon:project:instances:choice"
    ...
    def allowed(self, request, datum):
        try:
            ...
            cores_available = 0
            rams_available = 0
            #for zone in api.proxy.availability_zone_list(request, True):
            #    if zone.disabled:
            #        continue
            #    limits = api.proxy.project_absolute_limits(request, zone.id)

            #    instance_available = limits.maxTotalInstances \
            #        - limits.totalInstancesUsed
            #    instances_available += instance_available
...
class UpdateRow(tables.Row):
    ...
    def get_data(self, request, instance_id):
        instance = api.proxy.server_get(request, instance_id)
        zone = api.proxy.availability_zone_get(request, instance.availability_zone)
        if zone:
            instance.availability_zone = zone.name

        image_dict = dict((image.id, image) for image in
                image_utils.get_available_images(request, zone.id))
        ...
        gateway_dict = dict((gateway.hostname, gateway.vext_ip)
                for gateway in api.proxy.gateway_get_by_instance(request, instance_id))

        firewall = [(gateway_dict.get(f.hostname), f.gateway_port)
                for f in api.proxy.firewall_get(request, instance_id)]
        if firewall:
...
class StartInstance(tables.BatchAction):
    ...
    def action(self, request, obj_id):
        api.proxy.server_start(request, obj_id)
        #bill_api.update_container_status(request.session["user_id"], obj_id, "active")
...
class StopInstance(tables.BatchAction):
    ...
    def action(self, request, obj_id):
        api.proxy.server_stop(request, obj_id)
        #bill_api.update_container_status(request.session["user_id"], obj_id, "shutoff")
...
def get_power_state(instance):
    return POWER_STATES.get(getattr(instance, "power_state", 0), '')
...
class InstancesTable(tables.DataTable):
    ...
    class Meta:
        ...
        row_actions = (TerminateInstance, StartInstance, StopInstance,)
