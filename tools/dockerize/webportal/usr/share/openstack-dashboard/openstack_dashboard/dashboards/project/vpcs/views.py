...
def get_images(request):
    ...
    for zone in get_zones(request):
        ...
        for image in images:
            _images.append({'id': image.id, 'name': image.name,
...
def _get_servers(request):
    ...
    for server in api.proxy.server_list(request):
        image = image_dict.get(server.image, {})
        if zone_dict.has_key(server.availability_zone):
            server.availability_zone = zone_dict[server.availability_zone]
...
def server_format(request, server, image, firewall):
    ...
    disk_info = bill_api.get_disk_info(request.session["user_id"], server.id)
    if disk_info:
        data['disk_name'] = disk_info.disk_name
        data['disk_size'] = str(disk_info.disk_size) + "G" 
    return data
...
def create(request):
    ...
    kwargs = {'action': 'create', 'top': src['uid'], 'bottom': dst['uid']}
    api.proxy.security_group_update(request, **kwargs)
    return HttpResponse(status=204)

def delete(request):
    src, dst = validate(request.GET)
    kwargs = {'action': 'delete', 'top': src['uid'], 'bottom': dst['uid']}
    api.proxy.security_group_update(request, **kwargs)
    return HttpResponse(status=204)
...
def launch(request, zone_id):
    ...
    try:
        ...
        instance_info = {'container_id':instance.servers[0]['id'],
                         ...
                         'zonename':instance.servers[0]['availability_zone'],
                         'flavor_id':instance.servers[0]['flavor']
                    }
        if disk_info:
            disk_info['instance_id'] = instance.servers[0]['id']
            disk_info['zonename'] = instance.servers[0]['availability_zone']
        bill_api.new_container_count(user_id, instance_info, disk_info)
    except:
        LOG.error(traceback.format_exc())
        return HttpResponse(status=400)
    else:
...
def stop(request, instance_id):
    try:
        ...
        bill_api.update_container_status(request.session["user_id"], instance_id, "shutoff")
        status = 200
...
def start(request, instance_id):
    try:
        ...
        bill_api.update_container_status(request.session["user_id"], instance_id, "active")
        status = 200
    except HTTPClientError as e:
        status = 423
    except Exception:
...
def terminate(request, instance_id):
    ...
    try:
        ...
        bill_api.delete_container_count(request.session["user_id"], instance_id, instance_id)
        status = 200
    except Exception:
        LOG.error(traceback.format_exc())
        status = 400
