...
CONF = cfg.CONF
CONF.register_opt(cfg.IntOpt('timeout', default=20))
...
class GroupController(ControllerBase):
    ...
    def delete(self, _req, **kwargs):
        ...
        sid = body.get('sid')
        did = body.get('did')
        if any((not sid, not did)):
            return Response(status=400)
        self.app.group_delete(sid, did)
...
class GatewayState(dict):
    ...
    def _add(self, gateway):
        if not self._idc.has_key(gateway.idc_id):
            self._idc[gateway.idc_id] = set()

        self._idc[gateway.idc_id].add(gateway.datapath_id)
    ...
    def idc_get(self, idc):
        return self._idc.get(idc)
...
class PacketLib(db_base.Base):
    ...
    def init_flow(self, dp):
        self.packet_group.init_flow(dp, self.gateway_get(dp.id))

    def group_delete(self, sid, did):
        src = self.db.server_get(sid, gateway=True)
        dst = self.db.server_get(did, gateway=True)
        if any((not src, not dst, not src.Instance.address, not dst.Instance.address)):
