...
class PacketIPv4(PacketBase):
    ...
    def setup_ip_flow(self, msg, dp, in_port, src_key, pkt_ether, pkt_ipv4, pkt_tp,
        ...
        server_port = self.port_get(rdp, dst.id)
        if not server_port:
            return
    ...
    def _firewall(self, msg, dp, in_port, pkt_ether, pkt_ipv4, pkt_tp, server, gateway,
        ...
        def server_port_wrap(dpath):
            server_port = self.port_get(dpath, server.Instance.id)

            # Virtual machine may be poweroff
            if not server_port:
                raise exception.DevicePortNotFound(device=server.Instance.id)
        ...
        if server.Instance.host == gateway.hostname:
            ...
            input_local_actions = [
                    lofp_set(eth_src=server.Gateway['vint_mac']),
            ...
            output_local_actions = [
                    lofp_set(eth_src=server.Gateway['int_mac']),
    ...
    def run(self, msg, pkt_ether, pkt_ipv4, pkt_tp, src_gateway, **kwargs):
        ...
        if pkt_ipv4.proto == inet.IPPROTO_ICMP:
            src_key = socket.htons(self.filter_port(pkt_tp.id))
            LOG.error("ICMP_Id:%d",src_key)
            #src_key = pkt_tp.id
        else:
