...
class GetPasswordForm(BaseForm):
    ...
    def clean(self):
        ...
        if username and email:
            try:
                proxyclient().users.validate(username=username, email=email)
            except:
    ...
    def handle(self, request, data):
        ...
        url = urlparse.urlunparse((request.META['REQUEST_SCHEME'],
                                   ...
                                   None, None, None))
        try:
            proxyclient().users.update_key(username=username, email=hmail, url=url)
...
class ResetPasswordForm(BaseForm):
    ...
    def handle(self, request, data):
        try:
            proxyclient().users.resetpassword(data['uid'],
                                              data['username'],
                                              data['email'],
                                              key=data['tid'],
