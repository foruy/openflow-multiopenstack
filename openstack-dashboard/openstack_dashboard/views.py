...
from django_auth.forms import Login
from django_auth.user import set_session_from_user
...
def index(request):
    ...
    zones = [zone for zone in api.proxy.availability_zone_list(
             request, True) if not zone.disabled]
...
def checkdata(request):
    ...
    if post_data['checkonly_type'] == 'email' and \
        ...
        try:
            proxyclient().users.check(post_data['checkonly_type'], post_data['check_dest'])
        except:
...
def register(request):
    if request.method == 'POST':
        ...
        user = proxyclient().users.register(
